...
TINT = os.environ.get("HESPER_TINT_BIN", "/tmp/tint-build/tint")
OUT = os.path.join(HERE, "out")
os.makedirs(OUT, exist_ok=True)

trace = json.load(open(os.path.join(HERE, "out", "trace.json")))
...
for op in trace["ops"]:
    ...
    for g in op["bgs"]:
        for e in g["entries"]:
            ...
            binds.append([k, e["buf"], e["offset"], e["binding"]])
    ops_out.append({"p": pm, "plan": op.get("plan", ""), "grid": op["grid"], "binds": binds})
