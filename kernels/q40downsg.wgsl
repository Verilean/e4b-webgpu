...
// GROUPED MoE DOWN GEMM on subgroup matrices: grid.z = expert chunk, M-tile =
// the chunk's ≤MC entries (x rows = gegluSlots[flat entry]), N = the expert's
// OUT rows in 128-row strips, K = IN(704) in q4_0 block tiles. UNWEIGHTED
// output per entry to yraw[(tok*K+slot)][OUT]; wacc applies topkW and sums.
// Params: IN(704), OUT(2816), K, MC(8), TPREC
@group(0) @binding(0) var<storage, read> xh: array<vec4<f16>>;   // gegluSlots [M*K][IN/4]
...
@group(0) @binding(5) var<storage, read_write> yraw: array<f32>; // [M*K][OUT]
...
fn main(@builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_index) li: u32,
  ...
  let rowB = ${IN}u / 32u;
  let eb = ce * (${OUT}u * rowB);            // expert base (blocks)
  ...
  for (var kb: u32 = 0u; kb < rowB; kb = kb + 1u) {
    if (li < 32u) {
      let ent = ents[aEnt];
      if (ent != 0xFFFFFFFFu) {
        let ax = (ent * ${IN}u + kb * 32u + aCol) / 4u;   // x row = flat entry
        let v0 = xh[ax];
        let v1 = xh[ax + 1u];
  ...
  for (var s: u32 = 0u; s < 4u; s = s + 1u) {
    ...
    if (ent != 0xFFFFFFFFu) {
      ...
      yraw[ent * ${OUT}u + gn] = scr[sub][row * 8u + col];
      yraw[ent * ${OUT}u + gn + 1u] = scr[sub][row * 8u + col + 1u];
    }
  }
}
