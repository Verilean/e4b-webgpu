...
nohup "$CHROME" --headless=new --user-data-dir="$PROFILE" --no-first-run \
  ...
  --enable-dawn-features=disable_robustness${EXTRA_DAWN:+,$EXTRA_DAWN} \
  --disk-cache-size=1 --media-cache-size=1 \
  "http://127.0.0.1:8877/engine-dg.html?seed=${ENGINE_SEED:-12345}&prof=${ENGINE_PROF:-0}" >/dev/null 2>&1 &
SECS=0
