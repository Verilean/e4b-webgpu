...
nohup "$CHROME" --headless=new --user-data-dir="$PROFILE" --no-first-run \
  --enable-unsafe-webgpu --use-angle=metal --enable-dawn-features=dump_shaders,disable_symbol_renaming --enable-logging=stderr --v=1 \
  --disk-cache-size=1 --media-cache-size=1 \
  "http://127.0.0.1:8877/replay-dg.html" > /tmp/claude-503/chrome-msl.log 2>&1 &
SECS=0
until grep -qE "REPLAY (PASS|FAIL|EXCEPTION)" "$ROOT/harness/run.log" 2>/dev/null; do
  sleep 2; SECS=$((SECS+2))
